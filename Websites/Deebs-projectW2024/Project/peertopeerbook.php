...
$sql = "SELECT * FROM bookingusers WHERE EMAIL='$email' ORDER BY BOOK_ID DESC LIMIT 1";
...
if ($rows == null) {
    ...
    $car_id = $rows['CAR_ID'];
    $sql3 = "SELECT * FROM usercars WHERE CAR_ID='$car_id'";
...
<?php
    if ($rows['BOOK_STATUS'] == 'APPROVED') {
        ...
        echo '<button class="sign-agreement"><a href="feedback/feedbackuser.php">Rate your experience!</a></button>';
    } 
...
function returnCar(carId, bookId) {
    ...
    if (confirmation) {
        // Send AJAX request to userreturn.php to process car return
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "userreturn.php?id=" + carId + "&bookid=" + bookId, true);
